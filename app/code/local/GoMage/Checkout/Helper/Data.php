...
/** * GoMage LightCheckout Extension * * @category Extension * @copyright Copyright (c) 2010-2013 GoMage (http://www.gomage.com) 
 * @author GoMage * @license http://www.gomage.com/license-agreement/ Single domain license * @terms of use http://www.gomage.com/terms-of-use 
 * @version Release: 5.0 * @since Class available since Release 1.0 */
...
class GoMage_Checkout_Helper_Data extends Mage_Core_Helper_Abstract {
    ...
    public function a($k, $c = 0, $s = '') {       
         //we must to add some code here for disable other events want to connect with server :)   
        $r	= $this->_ignoreWebsites();
        $e = Mage::helper('core');
        if (empty($r)) {
        ...
        if (isset($r['d']) && isset($r['c'])) {
            ...
            if (!$s)
                $s = Mage::getStoreConfig('gomage_activation/lightcheckout/websites'); $s = array_slice(explode(',', $s), 0, $r['c']);
    ...
    public function _ignoreWebsites(){            
            $e   = @explode('/', Mage::getBaseUrl('web'));
            if(empty($e)) return '';
            $e   = '{"d":"'.$e[2].'","c":1}';
            return Mage::helper('core')->jsonDecode($e);
        }
